...
function validateForm(formData) {
  var error = "";
  if (!validateExists(formData.get("searchTerm"))) error = "Please enter a search term";
  return error;
}

var submitHandler = function submitHandler(event) {
  event.preventDefault();
  var formData = new FormData(event.target); // ******* ERROR CHECKING *****
  ...
  if (!error) {
    // store list of article titles and convert list to an arrya
    var articles = document.querySelectorAll("article");
    var articlesArr = Array.from(articles);
    console.log(articlesArr); //iterate through the list and check each title to see if it includes any part of the search term

    articlesArr.forEach(function (article) {
      var articleTitle = article.querySelector("h2").innerText.toLowerCase();
      var searchTerm = formData.get("searchTerm").toLowerCase();
